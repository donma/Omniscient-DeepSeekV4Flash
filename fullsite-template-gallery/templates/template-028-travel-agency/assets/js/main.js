document.addEventListener('DOMContentLoaded',function(){
  ...
  document.querySelectorAll('form').forEach(f=>{f.addEventListener('submit',function(e){e.preventDefault();alert('感谢您的填写！我们将尽快与您联系。');this.reset()})});
  const bt=document.createElement('button');bt.innerHTML='<i class="bi bi-arrow-up"></i>';bt.className='btn btn-primary position-fixed';bt.style.cssText='bottom:30px;right:30px;width:50px;height:50px;border-radius:50%;display:none;z-index:9999;align-items:center;justify-content:center;';document.body.appendChild(bt);
